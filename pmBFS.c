...
/* Run simulation */
void BFS(long double A[P_SIZE][P_SIZE][5], long double P[P_SIZE][6][2], long int swtch1, long int swtch2) {
...
#pragma omp parallel for schedule(static)
  for (i = 0; i < P_SIZE; i ++)
      ...
      for (j = lower; j < upper; j ++)
	{
	  if (P[i][5][swtch1] == 1.0)
	    {
	      P[j][0][swtch2] += A[i][j][0]+P[i][0][swtch1];
	      P[j][5][swtch2] = 1.0;
	      if (j > 0 && (j % 12) == 0 )
		{
		  P[i][5][swtch2] += 1.0;
		}
	      P[j][1][swtch2] += A[i][j][1]+P[i][1][swtch1];
	      P[j][2][swtch2] += A[i][j][2]+P[i][2][swtch1];
	      P[j][3][swtch2] += A[i][j][3]+P[i][3][swtch1];
	      P[j][4][swtch2] *= A[i][j][4]*P[i][4][swtch1];
	    }
	}
    }
  }
...
#pragma omp parallel for schedule(static)
  for (i = 0; i<P_SIZE; i++)
   {
    P[i][5][swtch1] = P[i][5][swtch2];
    P[i][0][swtch1] = P[i][0][swtch2];
    P[i][1][swtch1] = P[i][1][swtch2];
    P[i][2][swtch1] = P[i][2][swtch2];
    P[i][3][swtch1] = P[i][3][swtch2];
    P[i][4][swtch1] = P[i][4][swtch2];
...
int main(int argc, char** argv) {
  ...
  P = malloc(P_SIZE * sizeof(long double[6][2]));
  ...
  n_index = N%2;
  for (i = 0; i < P_SIZE; i++) {
    P2[i][0] = P[i][0][n_index];
    P2[i][1] = P[i][1][n_index];
    P2[i][2] = P[i][2][n_index];
    P2[i][3] = P[i][3][n_index];
      
    for (j = i; j < P_SIZE; j++) {
  	  if (((checker[j] == 0 && P[j][0][n_index] == P[i][0][n_index]) &&
          (P[j][1][n_index] == P[i][1][n_index] && P[j][2][n_index] == P[i][2][n_index])) &&
          (P[j][3][n_index] == P[i][3][n_index])) {
  	      P2[i][4] += P[j][4][n_index];
  	      checker[j] = 1;
  	  }
	  }
  }
